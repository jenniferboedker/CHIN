# script to generate a support vector machine (SVM) for a given data set
...
test_x = dataset_test[:, 2:121].astype(float)  # for each molecule/row use the feature info from col 2-119
test_y = dataset_test[:, 0].astype(int)
test_s = dataset_test[:, 1] #smiles
...
grid = GridSearchCV(estimator=estimator, param_grid=param_grid, n_jobs=1, scoring='roc_auc', cv=cv)
